...
def download_mokuro_files(manga_url: str, manga_name: str, session: requests.Session) -> None:
    """
    ...
    for anchor in soup.find_all('a', href=True):
        ...
        if href.lower().endswith('.mokuro'):
            ...
            volume_filename = volume_name if volume_name.lower().endswith('.mokuro') else f"{volume_name}.mokuro"
